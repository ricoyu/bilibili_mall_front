      ...
      <div class="content-area">
        ...
        <div class="table-container">
          ...
          >
            ...
            <el-table-column prop="attrName" label="属性名" width="150" show-overflow-tooltip />
            <el-table-column prop="searchType" label="可检索" width="95" align="center">
            ...
            <el-table-column prop="valueSelect" label="可选值" min-width="200" show-overflow-tooltip>
              <template #default="{ row }">
            ...
            <el-table-column prop="enable" label="启用" width="95" align="center">
              <template #default="{ row }">
            ...
            <el-table-column prop="catelogName" label="所属分类" width="140" show-overflow-tooltip>
              <template #default="{ row }">
                {{ row.catelogName || '-' }}
              </template>
            </el-table-column>
            <el-table-column prop="showDesc" label="快速展示" width="110" align="center">
              <template #default="{ row }">
    ...
    >
      ...
      >
        ...
        <el-form-item label="属性类型" prop="attrType">
          <el-input :value="'销售属性'" disabled style="width: 100%" />
        </el-form-item>
...
import { getCategoryTree, searchAttrs, createAttr, updateAttr, deleteAttr } from '../utils/api'
...
const loadAttrData = async (catelogId = null) => {
  ...
  try {
    ...
    ElMessage.error('加载销售属性数据失败：' + error.message)
    tableData.value = []
...
const handleEdit = async (row) => {
  ...
  // 调试：打印后端返回的原始数据
  console.log('编辑时的原始数据:', row)
  console.log('valueSelect值:', row.valueSelect)
  console.log('valueType值:', row.valueType, '类型:', typeof row.valueType)
  ...
  // 填充表单数据
  // 处理 valueType：后端返回的是中文字符串"多选"或"单选"
  let valueType = 0
  if (typeof row.valueType === 'number') {
    // 如果是数字，直接使用
    valueType = row.valueType === 1 ? 1 : 0
  } else if (typeof row.valueType === 'string') {
    // 处理字符串类型：后端返回的是中文"多选"或"单选"
    valueType = row.valueType === '多选' ? 1 : 0
  }
  
  // 填充表单数据（参考规格参数组件的做法，直接赋值）
  formData.value = {
    attrName: row.attrName || '',
    attrType: row.attrType !== undefined ? row.attrType : 0,
    valueType: valueType,  // 已转换为 0 或 1
    valueSelect: row.valueSelect || '',  // 直接使用后端返回的值
    icon: row.icon || '',
  ...
  console.log('填充后的 formData.valueSelect:', formData.value.valueSelect)
  console.log('填充后的 formData.valueType:', formData.value.valueType)
  console.log('row.valueSelect 原始值:', row.valueSelect)
  
  // 处理可选值数组（如果是多选且有值，转换为数组；单选时清空数组）
  ...
  if (formData.value.valueType === 0 && row.valueSelect) {
    ...
    console.log('nextTick 后再次设置 valueSelect:', formData.value.valueSelect)
  }
...
onMounted(() => {
  console.log('SaleAttrManagement 组件已挂载')
  loadCategoryTree()
  // 页面加载时调用接口
  loadAttrData(null)
})
